...
function App() {
  // axios.delete('https://server-for-vite-crud.onrender.com/users/5')
  const [Status, setStatus] = useState()
  const [Value, setValue] = useState()
  const [Users, setUsers] = useState()
  const getUsers = () => {
    axios.get("https://server-for-vite-crud.onrender.com/users")
      .then((response) => {
  ...
  const postUsers = () => {
    if (Value.length > 0) {
      axios.post("https://server-for-vite-crud.onrender.com/users", { "name": Value })
    }
    setValue('')
  }
  return (
                ...
                <table className="w-full text-left border border-collapse rounded sm:border-separate border-slate-200" cellspacing="0">
                  <tbody>
                    ...
                    {Users?.map((data, index) => {
                      return (
                        <tr>
                          <td className="h-12 px-6 text-sm transition duration-300 border-t border-l first:border-l-0 border-slate-200 stroke-slate-500 text-slate-500 text-gray-800">{index + 1}. {data.name}</td>
                        </tr>
                      )
                    })}
                  </tbody>
